...
* Does same job as the CMIP5_SST.py code, but reads .nc files using a different method by listing all file names and reading the files one by one (instead of loading them all using MFDATASET) - This is useful when the data are huge and the computer memory is low
